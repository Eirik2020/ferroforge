# Review History - Archived 2026-09-16

Archived from `docs/src/review.md` on 2026-09-16. Contains the "Earlier
Decision - Per-System Composition and Unified Target" and "Initial Walkthrough
and Backend Proofs" sections exactly as they stood at archival.

Binding decisions from this material were promoted into the active
`docs/src/review.md` before archival. What remains here is rationale, evidence,
implementation status, and superseded discussion - not active design authority.

---

## Earlier Decision - Per-System Composition and Unified Target

On 2026-09-14, the user clarified that each system must author `composition!`
together with its hardware init. Hardware interrupt tasks and multiple
independent task crates are required in the repair, and the legacy example
must keep working during migration. One unified target definition is a core
requirement: target facts must not be maintained independently across consumers.

The subsequent clarification establishes **firmware** as the term for a system.
Each firmware owns an app composition and handwritten init in its own Cargo
workspace, with Rust Analyzer type checking as a requirement. A backend crate
per family (for example STM32F4) originally owned hardware tasks and family
helpers together; the current decision above splits those responsibilities.
This earlier discussion assigned interrupt
bindings to hardware tasks and allowed an enum. The governing requirements above
now require an enum and confirm that hardware tasks require the binding.

The user has now confirmed that the family backend also owns the concrete
chip target definitions selected by firmware. The additional instruction,
"keep all firmware in a single firmware crate", was subsequently clarified:
`firmware/` is a grouping directory, not a compiled Cargo crate. All firmware
targets live below it, each with its own workspace containing composition and
handwritten init. Betaflight's target organization is the requested structural
reference; the [upstream comparison](composition-repair-plan.md#betaflight-structural-reference)
distinguishes that analogy from our Rust-specific checking requirements.

These are agreed requirements, not implemented behavior. The
[repair plan](composition-repair-plan.md) records the gaps, pending phases,
target ownership, and acceptance gates. The declaration-driven workspace layout
is agreed. After the authoring review,
remaining discussion covers Rust Analyzer preparation, automatic
checking-interface refresh, diagnostic coverage, and target-change handling.
Board-specific settings, Cargo inheritance,
host/target checking layout, helper type identity, and the interrupt enum
API then need explicit decisions. Exact declaration grammar and canonical target
representation remain unselected. Codex continues this review in ordinary
conversation, not pop-up questionnaires; this restriction applies to Codex only.

The user accepted the [workspace authoring layout](composition-repair-plan.md#agreed-declaration-driven-workspace-layout):
one target-checked authored package per firmware workspace, containing
composition and handwritten init, with shared host generation tooling rather
than a per-firmware host composer executable. Generated firmware remains an
isolated build artifact in the agreed design. Implementation is pending. The
[detailed Rust Analyzer proposal](composition-repair-plan.md#rust-analyzer-checking-and-refresh---detailed-proposal)
separates direct checking, generated interfaces, and final RTIC checks. Review
correction: typed task-local spawn mocks, composition-derived init spawn mocks,
and authored-source checking tests already exist. Reuse them in the new layout;
do not reopen the mock design or claim it is unimplemented. First establish any
actual refresh gap using the existing generator and macro loading mechanism.
A build hook remains an unselected automation option, not a requirement.

The earlier six-phase record describes bounded backend proofs. It does not
establish completion of the intended composition authoring interface.

## Initial Walkthrough and Backend Proofs
...
acceptance criteria. The centralized system layout, two explicit-Rust
composition frontends, and the bounded Phase 5 pipeline with real
source/composition/init/render/check/link failure proofs are implemented. Phase
6 now proves that both systems consume `tasks/blinky` unchanged while owning
different init, instance, resource, and configuration bindings. The required
next work is now the per-system composition and unified-target repair above;
it is not an optional frontend follow-on.
...
frontend ergonomics and additional target profiles remain work. The
...
not complete generalized frontend ergonomics or broader target portability. The
...
Phase 6 adds `systems/nucleo-f401re-fast-blink`. Its separate init and
composition map the unchanged reusable definitions to `heartbeat` and
`diagnostics`, rename all three resources, bind a 125 ms period, and pass the
same ARM check/release-link pipeline. Reusing the same board is deliberate and
does not claim portability to another MCU or HAL.
...
composition-generated checking interfaces are also agreed. Renderer integration
and verification are implemented for the bounded standalone scope. Broader API
coverage and the required firmware composition repair remain work; the initial
proofs do not establish completion of either.
...
| 6 | Implementation order and acceptance criteria | Bounded Phase 2 through Phase 6 backend gates met; required composition/unified-target repair pending |
...
- The initial walkthrough focused on SW tasks using real `embedded-hal`
  interfaces. HW tasks using chip HALs such as `stm32f4xx-hal` were deferred
  from those backend proofs; they are required by the current repair.
...
- **Resource implementation:** the generic checking expansion proves the initial
  ...
  sibling-module original-source emission, contained relative paths, authored
  rustc/Rust Analyzer diagnostics, and bounded firmware integration now have
  recorded proofs. Broader path forms and the new firmware workspace remain
  work; the focused IDE evidence does not establish complete diagnostic coverage.
...
- **Checking guarantees:** enforce declared bounds and RTIC-compatible borrowing
  ...
  check. The initial expansion and concrete substitution have compiler proofs;
  the bounded complete applications also pass ARM checks and release linking.
  Broader task shapes and cross-family integration remain unproved.
...
HW tasks are required in the current repair, subject to the interrupt
clarification above. Host simulation remains deferred. Extend the API only as
concrete uses require.
...
`blink::spawn(...)`, task configuration, and monotonic startup. The legacy path
supplies these through the containing app declaration in
[legacy system source](prototype.md#legacy-system-source); the standalone path
generates its init checking interfaces from composition.
...
`ferroforge` dependency remains in that authored check manifest and is excluded
from the generated firmware manifest. The macro parses the generated
...
agreed `u32`/1 kHz delay. Its composition-generated init checker supplies
`start(SYST, u32)`, and the standalone init transplant rewrites that call to the
real backend. The startup/signature gap remains on the legacy app-backed path.
...
Init support must likewise preserve its source environment. The standalone
root-support discovery and transplant have a native-HAL ARM proof; child init
modules and broader discovery/access cases remain outside the implemented scope.
...
standalone init transplant are now implemented. Standalone manifest emission
and both system pipelines have ARM check and release-link proofs. Relative
imports, mapped calls and paths inside other macro tokens, overlapping module
selection, and the required firmware composition repair remain explicit gaps.
...
Current source cleanup recognizes the literal `ferroforge` crate name. The
metadata filters other listed dependencies from manifests, but does not yet
drive removal or diagnosis of their imports in transplanted source.
...
firmware followed by demonstrated reuse, not exhaustive mock coverage. The
bounded acceptance bar is now met by the two Nucleo-F401RE systems. Keep host
simulation separate from compile-only mock support; this result does not extend
the supported APIs or prove another HAL/MCU target.
...
configuration/profile rejection and orchestration failure propagation. All
bounded backend phases have recorded proofs. The per-system `composition!`
requirement remains unfinished and is tracked in the repair plan; broader
target portability remains separate.
