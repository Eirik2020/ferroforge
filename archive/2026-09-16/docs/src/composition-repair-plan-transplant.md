# composition-repair-plan - Archived 2026-09-16 (transplant design)
...
| G4 | Naming an interrupt the selected chip does not have fails the authored package's own check, on the authored line, without FerroForge maintaining an interrupt list. |
...
2. **Cargo centralization - decided 2026-09-16:** independently authored
   ...
   firmware's choice.
3. **Target authoring format - revised 2026-09-16 under the call-through model:**
   with nothing transplanted there is no compile-time consumer of chip data, so
   the resolved host model and the backend crate both fall away. Per G2a a
   backend is build-time data - memory layout, probe identity, Rust target
   triple, device path, platform crate selections - read by the CLI to emit
   `memory.x`, `.cargo/config.toml` and `Embed.toml` and to scaffold the
   firmware manifest. TOML is sufficient and must not name interrupts, because
   per G4 the device's own enum is the only interrupt list.
...
7. **Shared helper integration and interrupt API - closed 2026-09-16:** nothing
   to build. The firmware's `device =` selects the PAC, RTIC resolves `binds`
   against that device's own `Interrupt` enum, and an interrupt the chip does not
   have is a compile error on the authored line - verified against
   `NOT_A_REAL_INTERRUPT`, which fails with `no variant ... found for enum
   Interrupt`. FerroForge neither owns nor validates an interrupt list, and
   nothing interrupt-shaped belongs in the backend data. Still open: the family
   helper-crate dependency with shared
