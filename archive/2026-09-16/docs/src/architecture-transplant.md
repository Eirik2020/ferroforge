# architecture - Archived 2026-09-16 (transplant design)
...
Per G2a a family backend is chip-family data, not a crate: memory layout, probe
identity, Rust target triple, device path, and the platform crate selections,
consumed before a build rather than depended on by the firmware. Reusable
hardware tasks and their helpers live in their own crates alongside it, and
common reusable crates contain software tasks. Both task kinds use the same
RTIC-like definition and composition model.

Per G4, an interrupt binding names a variant of the selected device's own
interrupt enum, so an interrupt the chip does not have is a compile error on
the authored line. FerroForge defines no interrupt list of its own. Per G3,
hardware tasks carry the interrupt binding and software tasks do not.
Helper sharing details remain open. Concrete target
