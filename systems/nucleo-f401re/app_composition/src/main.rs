...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rendered = ferroforge_nucleo_f401re_composer::run_nucleo_f401re_pipeline(
        repository_root,
        &system_dir.join(".ferroforge/init-check"),
        &system_dir.join("gen_app"),
    )?;

    println!("checked and built {}", rendered.firmware.root.display());
    println!(
        "rendered {}",
        rendered.init_check.interface_source.display()
    );
    Ok(())
}
